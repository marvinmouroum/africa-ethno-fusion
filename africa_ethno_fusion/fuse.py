...
from . import crosswalk, schema, sources
...
def build(
    ...
    *,
    ...
    Returns {"groups": GeoDataFrame, "traits": DataFrame, "links": DataFrame}.
    """
    ...
    return {"groups": groups, "traits": traits, "links": links}
...
def export(frames: dict, outdir: str, fmt: str = "all") -> None:
    ...
    if fmt in ("gpkg", "all"):
        # One GeoPackage with the spine as a spatial layer.
    ...
    print(f"  groups: {len(groups):>6} records  ({groups['source'].nunique()} sources)")
