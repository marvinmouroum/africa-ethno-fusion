...
Layers (toggle in the top-right control):
  * Murdock ethnic territories (~1900), enriched with key Ethnographic Atlas traits
  * GeoEPR modern settlement areas (latest period per group)
  * D-PLACE Ethnographic Atlas society points (full trait popup)
Open the result by double-clicking out/map.html.
"""
...
piv = tt.pivot_table(index="ea_society_id", columns="lab", values="value_label", aggfunc="first")

m = folium.Map(location=[3, 20], zoom_start=4, tiles="CartoDB positron",
               control_scale=True)
...
folium.GeoJson(
    ...
    style_function=lambda f: {"fillColor": "#3186cc", "color": "#13496f",
                              "weight": 0.5, "fillOpacity": 0.35},
    highlight_function=lambda f: {"weight": 2, "fillOpacity": 0.6},
    tooltip=folium.GeoJsonTooltip(
        fields=show_cols,
        aliases=["Group", "EA id", "Agriculture", "Settlement",
                 "Political complexity", "Descent", "Class"],
...
ge["period"] = ge["period_from"].astype("Int64").astype(str) + "–" + ge["period_to"].astype("Int64").astype(str)
folium.GeoJson(
    ge[["name", "settlement_type", "period", "geometry"]],
    name="GeoEPR settlement areas (modern)",
    show=False,
    style_function=lambda f: {"fillColor": "#cc5b31", "color": "#7a3413",
                              "weight": 0.5, "fillOpacity": 0.30},
    tooltip=folium.GeoJsonTooltip(fields=["name", "settlement_type", "period"],
                                  aliases=["Group", "Settlement", "Period"]),
).add_to(m)

# --- D-PLACE society points with full trait popup ---
fg = folium.FeatureGroup(name="Ethnographic Atlas societies (points)", show=False)
...
for _, r in dp.iterrows():
    ...
    items = "".join(
        ...
        for v in rows.itertuples() if pd.notna(v.value_label)
    )
    html = (f"<b>{r['name']}</b> ({soc})<br>glottocode: {r['glottocode']}<br>"
            f"<table style='font-size:11px'>{items}</table>")
    folium.CircleMarker(
        [r.geometry.y, r.geometry.x], radius=4, color="#2b7a2b",
...
if not jp.empty:
    ...
    for _, r in jp.iterrows():
        ...
        html = (f"<b>{r['name']}</b><br>country: {r['country_iso3']}<br>"
                f"language (ISO 639-3): {r['iso639_3']}<br>"
                f"population: {pop}<br>religion: {r['primary_religion']}")
        folium.CircleMarker(
            [r.geometry.y, r.geometry.x], radius=3, color="#7a2b7a",
            fill=True, fill_opacity=0.8,
            popup=folium.Popup(html, max_width=320), tooltip=r["name"],
        ).add_to(cj)
    fgj.add_to(m)

folium.LayerControl(collapsed=False).add_to(m)

title = ("<div style='position:fixed;top:10px;left:50px;z-index:9999;background:white;"
         "padding:6px 12px;border-radius:6px;box-shadow:0 1px 4px rgba(0,0,0,.3);"
         "font-family:sans-serif;font-size:13px'>"
         "<b>Africa — fused ethnographic map</b><br>"
         "<span style='color:#3186cc'>■</span> Murdock territories &nbsp;"
         "<span style='color:#cc5b31'>■</span> GeoEPR &nbsp;"
         "<span style='color:#2b7a2b'>●</span> EA societies &nbsp;"
         "<span style='color:#7a2b7a'>●</span> Joshua Project"
         "</div>")
m.get_root().html.add_child(folium.Element(title))

m.save(f"{OUT}/map.html")
print(f"wrote {OUT}/map.html  ({len(mur)} Murdock polys, {len(ge)} GeoEPR polys, {len(dp)} EA points)")
